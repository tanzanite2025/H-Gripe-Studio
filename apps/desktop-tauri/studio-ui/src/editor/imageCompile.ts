...
export function compileImageAdjustments(doc: ImageDocument): GradeDoc | null {
  ...
  for (const l of doc.layers) {
    ...
    if (l.layer.kind === "pixel") {
      ...
      const invertCount = l.layer.edits.filter((op) => op.type === "invert" && !op.disabled).length;
      if (l.visible && invertCount % 2 !== 0) return null;
      continue;
    }
    if (l.layer.adjustment && !l.layer.ops && !canLowerAdjustmentToGrade(l.layer.adjustment)) return null;
