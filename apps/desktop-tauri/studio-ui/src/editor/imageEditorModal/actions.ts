// Image editor state actions: the reducer over `imageEditorState.ts`'s pure
// document-editing functions, shared by the editor shell and its panels.
...
export type ImageEditorAction =
  ...
  | { type: "op"; op: ImageEditOperation & EditOpBase }
...
export type ImageEditorDispatch = (action: ImageEditorAction) => void;
...
export function imageEditorReducer(state: EditState, action: ImageEditorAction): EditState {
  switch (action.type) {
    case "stroke":
