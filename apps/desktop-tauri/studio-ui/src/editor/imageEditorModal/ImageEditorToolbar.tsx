...
// shortcut letter as a corner badge (from the image-editor scope's `toolCombo`
// table, falling back to the slot's reserved PS letter) alongside the
// tooltip.

import { useContext, useEffect, useRef, useState } from "react";
import { PS_TOOL_SECTIONS, imageEditorTool, type ImageEditorTool } from "../imageEditorTools";
...
function isActive(mt: ImageEditorTool, toolId: string): boolean {
  return toolId === mt.id && (mt.kind !== "global" || isPreviewableOp(mt.id));
}

export function ImageEditorToolbar({ toolId, onToolClick, hiddenSlotIds = [], faces, onPickFace, paintMode, fgColor, bgColor, onPickColor, onSwapColors, onResetColors }: ImageEditorToolbarProps) {
  const t = useT();
  ...
  const pick = (key: string, mt: ImageEditorTool) => {
      ...
      {PS_TOOL_SECTIONS.map((section, si) => (
        <div key={si} className="mask-tool-group">
          {section.map((slot) => {
            ...
            const key = slot.id;
            const tools = slot.variants.map((id) => imageEditorTool(id)).filter((mt): mt is ImageEditorTool => mt != null);
