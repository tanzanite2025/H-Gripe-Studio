// Magnetic-lasso edge snapping: an edge-magnitude map built from a
// gesture-scoped selection-assist read, and a snap that pulls a point to the
// strongest edge nearby. The assist read must resolve from the active editable
// layer's materialized pixels; it is not the Ctrl+J pixel-copy source.

/** Edge-magnitude map over the assist-read window. `offX`/`offY` map
