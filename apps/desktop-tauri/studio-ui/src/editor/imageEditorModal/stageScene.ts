...
import { activeTargetKind, type ImageEditorDocument } from "../../contracts/imageEditorDocument";
import { isBrushOp, isPathOp, type EditPathPoint } from "../../contracts/imageEditOps";
import { layerOpStacks } from "../imageEditorState";
...
export function paintStage(ctx: CanvasRenderingContext2D, args: StagePaintArgs): void {
  ...
  const sceneFrame = frame ?? { x: 0, y: 0, w: dims.w, h: dims.h };
  const selectionScene = buildSelectionOverlayScene(selectionDraft, activeSelection);
  ctx.clearRect(0, 0, sceneFrame.w, sceneFrame.h);

  if (overlayOnly) {
    // Transparency preview: dark backdrop so the mask reads clearly.
    ctx.fillStyle = "#0c0e14";
    ctx.fillRect(0, 0, sceneFrame.w, sceneFrame.h);
  }
  ctx.save();
  ctx.translate(-sceneFrame.x, -sceneFrame.y);
