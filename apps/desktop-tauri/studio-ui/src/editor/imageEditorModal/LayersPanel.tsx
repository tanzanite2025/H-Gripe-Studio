...
function LayerThumb({ layer, dims }: { layer: ImageEditorLayer; dims: { w: number; h: number } }) {
...
export function LayersPanel({
  ...
  const activeStudioTarget = useMemo<StudioTarget>(() => {
    const ref = { canvasId: "image-editor-stage", documentId: imagePath ?? "active-document" };
  ...
  const mergeable = (indices: number[]) =>
    indices.length >= 2 && indices.every((i) => layers[i]?.kind === "pixel" && !layers[i]?.locked);
