...
import {
  beginSelectedLayerMove,
  commitSelectedLayerMove,
  updateSelectedLayerMove,
} from "../layerMoveInteraction";
...
export function transformDown(env: PointerEnv, g: PointerGestures, pt: Pt): void {
  ...
  } else if (env.workspace !== "image" || env.canStartSelectedLayerMove(pt)) {
    beginSelectedLayerMove(env, g, pt);
  }
  env.forceRedraw();
}
...
export function transformMove(env: PointerEnv, g: PointerGestures, e: React.PointerEvent): boolean {
  ...
  if (g.moveDrag) {
    return updateSelectedLayerMove(env, g, e);
  }
  return false;
}
...
export function transformUp(env: PointerEnv, g: PointerGestures): boolean {
  ...
  if (g.moveDrag) {
    return commitSelectedLayerMove(env, g);
  }
  return false;
}
