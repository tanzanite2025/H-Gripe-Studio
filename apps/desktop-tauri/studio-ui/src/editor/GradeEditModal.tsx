...
// The grading dialog: an op stack over the connected image with a live
// preview. The preview prefers the backend `grade_preview` command (the
// hgripe-grade kernel — GPU when built with `grade-gpu`, else its CPU
// reference path); outside Tauri, or when the command fails, it falls back to
// the in-webview TS mirror over the thumbnail underlay, so the dialog stays
// usable in browser preview.
...
/** The op kinds the dialog can add, in menu order. */
const ADDABLE_OPS = [
  "exposure",
  "white_balance_k",
  "contrast",
...
// Dialog labels for op kinds. Ops authored elsewhere (e.g. curves from a
...
/** Parse the node's `grade_doc` param into the first layer's op stack. */
      ...
      <div className="media-viewer grade-edit" onClick={(e) => e.stopPropagation()}>
          ...
          <span className="media-viewer-name" title={title}>
            ...
            {backend ? (
              <span className="muted">
                {" "}
                · {((key) => (key ? t(key) : backend))(BACKEND_LABEL_KEYS[backend])}
              </span>
            ) : null}
          </span>
          <div className="media-viewer-actions">
            <button className="primary" onClick={handleApply} title={t("grade.applyTitle")}>
              {t("grade.apply")}
            </button>
        ...
        <div className="mask-edit-body">
          <div className="crop-edit-stage-wrap">
            ...
            <small className="muted">{previewError ?? t("grade.previewHint")}</small>
          </div>
