// Image Editor tool registry.
//
// The Image Editor modal renders its toolbar from this registry rather than
// hard-coding buttons. A `planned` tool renders greyed ("coming soon") and is
// not selectable. This mirrors the frozen contract in
// `docs/cards/subject-mask-matte.md` (§ "Image Editor tool registry").
...
export const IMAGE_EDITOR_TOOLS: readonly ImageEditorTool[] = [
 ...
 * flyout of tool variants. `variants` lists `IMAGE_EDITOR_TOOLS` ids in flyout
 ...
 * live in the right-panel Mask Ops group (`WHOLE_MASK_OPERATION_TOOL_IDS`), not here (plan step 4).
 */
 ...
 */
export const IMAGE_EDITOR_TOOL_GROUPS: readonly (readonly string[])[] = PS_TOOL_SECTIONS.map(
 ...
 */
export const IMAGE_EDITOR_TOOL_SLOTS: readonly (readonly (readonly string[])[])[] =
 ...
 */
export const WHOLE_MASK_OPERATION_TOOL_IDS: readonly string[] = ["invert", "fill_holes", "smooth", "grow", "shrink", "feather", "blur", "sharpen"];

export const READY_IMAGE_EDITOR_TOOLS = IMAGE_EDITOR_TOOLS.filter((t) => t.status === "ready");
export const PLANNED_IMAGE_EDITOR_TOOLS = IMAGE_EDITOR_TOOLS.filter((t) => t.status === "planned");

export function imageEditorTool(id: string): ImageEditorTool | undefined {
  return IMAGE_EDITOR_TOOLS.find((t) => t.id === id);
}

/** The paint targets a tool can act on (defaults to its implicit target). */
export function toolTargets(tool: ImageEditorTool): readonly PaintTarget[] {
...
export const DEFAULT_TOOL_ID = READY_IMAGE_EDITOR_TOOLS[0]?.id ?? "brush";
