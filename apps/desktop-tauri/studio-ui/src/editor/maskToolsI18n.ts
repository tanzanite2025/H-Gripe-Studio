...
export const MASK_TOOL_ZH: Record<string, { label: string; hint: string }> = {
  ...
  polygon_lasso: { label: "多边形套索", hint: "多边形套索：沿主体点击直线段（规划中）。" },
  magnetic_lasso: { label: "磁性套索", hint: "磁性套索：拖拽时路径自动吸附附近边缘（规划中）。" },
  object_select: { label: "对象选择", hint: "对象选择：框选后由模型自动生成框内对象的蒙版（规划中）。" },
  quick_select: { label: "快速选择", hint: "快速选择：涂抹时选区沿相似边缘扩展（规划中）。" },
  perspective_crop: { label: "透视裁剪", hint: "透视裁剪：拖出四边形并拉直为矩形（规划中）。" },
  color_sampler: { label: "颜色取样器", hint: "颜色取样器：固定最多四个持续显示的颜色读数（规划中）。" },
  ruler: { label: "标尺", hint: "标尺：拖拽测量距离和角度（规划中）。" },
  remove: { label: "移除工具", hint: "移除工具：刷过物体后由模型填补该区域（规划中）。" },
  healing_brush: { label: "修复画笔", hint: "修复画笔：Alt+点击设定源点，涂抹时融合覆盖瑕疵（规划中）。" },
  ...
  pencil: { label: "铅笔", hint: "铅笔：硬边缘无抗锯齿的绘制笔触（规划中）。" },
  ...
  background_eraser: { label: "背景橡皮擦", hint: "背景橡皮擦：擦除画笔中心取样到的颜色（规划中）。" },
  magic_eraser: { label: "魔术橡皮擦", hint: "魔术橡皮擦：点击擦除相似颜色，相当于魔棒+删除（规划中）。" },
  paint_bucket: { label: "油漆桶", hint: "油漆桶：用选区漫水填充相似颜色（规划中）。" },
  sponge: { label: "海绵", hint: "海绵：涂抹局部增加或降低饱和度（规划中）。" },
  freeform_pen: { label: "自由钢笔", hint: "自由钢笔：徒手绘制路径，锚点自动添加（规划中）。" },
  curvature_pen: { label: "弯度钢笔", hint: "弯度钢笔：点击各点后自动拟合平滑曲线（规划中）。" },
  type_horizontal: { label: "横排文字", hint: "横排文字：点击放置可编辑文本（规划中）。" },
  type_vertical: { label: "直排文字", hint: "直排文字：点击放置竖排可编辑文本（规划中）。" },
  path_select: { label: "路径选择", hint: "路径选择：选择并移动整条已提交路径（规划中）。" },
  direct_select: { label: "直接选择", hint: "直接选择：选择并移动路径上的单个锚点（规划中）。" },
};
