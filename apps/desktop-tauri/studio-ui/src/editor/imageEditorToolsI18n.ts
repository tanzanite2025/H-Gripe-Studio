// Simplified-Chinese overlay for the Image Editor tool registry (imageEditorTools.ts).
//
// IMAGE_EDITOR_TOOLS stays the English source of truth (the contract table in
...
import type { ImageEditorTool } from "./imageEditorTools";

export const IMAGE_EDITOR_TOOL_ZH: Record<string, { label: string; hint: string }> = {
  brush: { label: "画笔", hint: "把蒙版涂进来。" },
  eraser: { label: "橡皮", hint: "把蒙版擦掉。" },
  point: {
    label: "点选",
    hint: "左键点击以包含，右键点击以排除；自动选择时由内置确定性选择器使用这些点。",
  },
  ...
  object_select: { label: "对象选择", hint: "对象选择：拖一个框，运行时由内置确定性分割器为框内对象生成蒙版。" },
  ...
  content_aware_move: { label: "内容感知移动", hint: "内容感知移动：套索一块区域后拖到新位置——区域移过去，原位置的空洞由周围修复填补（可随时改参的一步）。" },
  red_eye: { label: "红眼", hint: "红眼：点击红色反光——相连的红色主导区域漫水并入蒙版。" },
  pencil: { label: "铅笔", hint: "铅笔：硬边缘笔触——硬度与流量锁定 100% 的画笔。" },
  color_replacement: { label: "颜色替换", hint: "颜色替换：保留纹理的同时涂上新的色相（规划中——灰度蒙版上颜色无语义）。" },
  mixer_brush: { label: "混合器画笔", hint: "混合器画笔：像湿颜料一样混合颜色（规划中——灰度蒙版上颜色无语义）。" },
  pattern_stamp: { label: "图案图章", hint: "图案图章：把重复的棋盘图案涂入蒙版（可随时改参的一步）。" },
  art_history_brush: { label: "历史艺术画笔", hint: "历史记录艺术画笔：以确定性抹动把区域还原到图层初始状态的风格化笔触（可随时改参的一步）。" },
...
export function localizeTool(tool: ImageEditorTool, lang: Lang): { label: string; hint: string } {
  if (lang !== "zh") return { label: tool.label, hint: tool.hint };
  const tr = IMAGE_EDITOR_TOOL_ZH[tool.id];
