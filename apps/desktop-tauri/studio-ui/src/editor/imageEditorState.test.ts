...
const stackPaths = (doc: ImageEditorDocument) => activeOps(doc).filter(isPathOp);
const stackBrushStrokes = (doc: ImageEditorDocument) => activeOps(doc).filter(isBrushOp);
const stackOperations = (doc: ImageEditorDocument) => activeOps(doc).filter(isImageEditOperation);
...
describe("imageEditorState reducer-style helpers", () => {
