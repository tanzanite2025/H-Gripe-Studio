// Pure edit-state model for the Image Editor.
//
// The modal owns an `EditState` (the current `ImageEditorDocument` plus an undo/redo
...
// and the React component stays a thin view. The committed `ImageEditorDocument` is
...
export const IMAGE_EDITOR_STATE_SCHEMA = "hgripe.imageEditorState.v1";

export interface PersistedImageEditorState {
  schema: typeof IMAGE_EDITOR_STATE_SCHEMA;
  version: 1;
  current: ImageEditorDocument;
  past: ImageEditorDocument[];
  future: ImageEditorDocument[];
...
export function initEditState(initial?: unknown): EditState {
  if (isPersistedImageEditorState(initial)) {
    return {
...
function isPersistedImageEditorState(value: unknown): value is PersistedImageEditorState {
  if (!value || typeof value !== "object") return false;
  const v = value as { schema?: unknown; version?: unknown; current?: unknown; past?: unknown; future?: unknown };
  return v.schema === IMAGE_EDITOR_STATE_SCHEMA && v.version === 1 && Array.isArray(v.past) && Array.isArray(v.future);
}

export function serializeEditState(state: EditState): PersistedImageEditorState {
  return {
    schema: IMAGE_EDITOR_STATE_SCHEMA,
    version: 1,
    current: cloneImageEditorDocument(state.current),
    past: state.past.slice(-MAX_HISTORY).map(cloneImageEditorDocument),
    future: state.future.slice(-MAX_HISTORY).map(cloneImageEditorDocument),
  };
}

/**
 * Coerce an arbitrary stored `edit_paths` value into a well-formed version-3
 * `ImageEditorDocument`. A version-3 value loads directly; a version-2 value (one
 ...
 */
export function normalizeEditPaths(value: unknown): ImageEditorDocument {
  if (!value || typeof value !== "object") return emptyImageEditorDocument();
  const v = value as {
    version?: unknown;
    layers?: unknown;
    active?: unknown;
  ...
  if (version >= 3) {
    ...
    const layers = storedLayers
      ...
      .filter((l): l is ImageEditorLayer => l !== null);
...
function normalizeLayer(value: unknown, validGroupIds?: ReadonlySet<string>): ImageEditorLayer | null {
  ...
  return {
    ...
    kind: isAdjustment ? "adjustment" : "pixel",
...
export function cloneImageEditorDocument(doc: ImageEditorDocument): ImageEditorDocument {
  return typeof structuredClone === "function"
    ? structuredClone(doc)
    : (JSON.parse(JSON.stringify(doc)) as ImageEditorDocument);
}
...
// unbounded in memory.
function commit(state: EditState, next: ImageEditorDocument): EditState {
  const past = [...state.past, cloneImageEditorDocument(state.current)];
  if (past.length > MAX_HISTORY) past.shift();
  return { current: cloneImageEditorDocument(next), past, future: [] };
}

function chronologicalDocuments(state: EditState): ImageEditorDocument[] {
...
export function jumpToHistorySnapshot(state: EditState, index: number): EditState {
  ...
  return {
    current: cloneImageEditorDocument(docs[index]),
    past: docs.slice(0, index).map(cloneImageEditorDocument),
    future: docs.slice(index + 1).reverse().map(cloneImageEditorDocument),
  };
}

function snapshotLabel(previous: ImageEditorDocument | null, doc: ImageEditorDocument): string {
...
function lastVisibleOp(doc: ImageEditorDocument): EditOp | null {
...
export function activeOps(doc: ImageEditorDocument): EditOp[] {
...
export function activeLayerLocked(doc: ImageEditorDocument): boolean {
  const layer = activeLayer(doc);
...
function withActiveOps(doc: ImageEditorDocument, ops: EditOp[]): ImageEditorDocument {
...
export function addOperation(state: EditState, op: ImageEditOperation): EditState {
  ...
  if (op.type === "transform" && activeLayer(state.current)?.linked) {
    return commit(state, {
      ...doc,
      layers: doc.layers.map((l, i) =>
        i !== doc.active && l.linked && l.kind === "pixel" && !l.locked ? { ...l, ops: [...l.ops, { ...op }] } : l,
      ),
    });
  }
...
export function updateOpAmount(state: EditState, index: number, amount: number): EditState {
  const ops = activeOps(state.current);
  const op = ops[index];
  if (!op || !isImageEditOperation(op)) return state;
...
export function updateOpTransform(state: EditState, index: number, params: TransformParams): EditState {
  const ops = activeOps(state.current);
  const op = ops[index];
  if (!op || !isImageEditOperation(op) || op.type !== "transform") return state;
...
export function addLayer(state: EditState, name?: string): EditState {
  const layers = [...state.current.layers, emptyPixelLayer(name ?? `Layer ${state.current.layers.length + 1}`)];
...
export function layerOpStacks(layer: ImageEditorLayer): { target: LayerTargetKind; ops: EditOp[] }[] {
...
export function withExplicitBaseSource(state: EditState, source: LayerImageSource): EditState {
  const migrate = (doc: ImageEditorDocument): ImageEditorDocument => {
...
function withLayer(state: EditState, index: number, patch: Partial<ImageEditorLayer>): EditState {
  const layer = state.current.layers[index];
...
function withoutLayerGroupId(layer: ImageEditorLayer): ImageEditorLayer {
  const next = { ...layer };
  delete next.groupId;
  return next;
}
...
export function mergeLayers(state: EditState, indices: number[]): EditState {
  ...
  if (sorted.some((i) => layers[i].kind !== "pixel" || layers[i].locked)) return state;
  const [target, ...rest] = sorted;
  const merged: ImageEditorLayer = {
    ...layers[target],
...
export function clearEdits(state: EditState): EditState {
  ...
  return commit(state, emptyImageEditorDocument());
}
...
export function undo(state: EditState): EditState {
  ...
  return { current: cloneImageEditorDocument(previous), past, future: [...state.future, cloneImageEditorDocument(state.current)] };
}
...
export function redo(state: EditState): EditState {
  ...
  return { current: cloneImageEditorDocument(next), past: [...state.past, cloneImageEditorDocument(state.current)], future };
}
...
export function isEmpty(doc: ImageEditorDocument): boolean {
  return (
    doc.layers.length === 1 &&
...
export function editCount(doc: ImageEditorDocument): number {
  return (
