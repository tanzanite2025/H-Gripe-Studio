...
with 16-bit PNG/TIFF-with-ICC output. (P5's Python-bridge parity half is
historical: the bridge was deleted in Phase 7, #314 — the native path is the
only implementation now.) See
...
landed, and P5 (cross-engine parity) closed the loop before the Python side
was retired in Phase 7 (#314). The *Open decisions* below are all closed too
— the initiative is complete, and the native chain is the only implementation.

## Dependency and vendoring policy

The colour pipeline is intentionally **locked** and the core colour-management
engine is now vendored. Cloud-side updates must use the repository copy, not an
implicit upstream crate release:
...
| ICC transforms in Rust | `third_party/moxcms` (`moxcms = 0.8.1` with `options`) | Yes | Resolved through the workspace `[patch.crates-io]`. The validated settings live in `studio/color/cmyk_transform.rs`: tetrahedral interpolation, high barycentric weights for the 8-bit path, default/low weights for the 16-bit ProPhoto path because moxcms `High` currently collapses that LUT path. |
...
repo. Today the ProPhoto profile is generated by the vendored moxcms fork and
embedded from that generated byte stream. If the cloud side changes
`third_party/moxcms`, `zune-*`, `tiff`, `image`, or `png`
behaviour, it must update this document, `Cargo.lock`, and the golden tests
together. The minimum checks are:
...
`moxcms` is no longer consumed directly from crates.io. The fork lives in
`third_party/moxcms` with its own `VENDOR.md`; upstream upgrades are deliberate
source replacements plus local patch re-application, never an implicit Cargo
version bump.
...
> All phases are complete; this section is a changelog. The Python-side
> artifacts P5 delivered (`wide_gamut.py`, the pytest parity suite, the
> Python cpu engines) were later **deleted in Phase 7 (#314)** — read those
> mentions as historical; the native Rust path is the only implementation.
