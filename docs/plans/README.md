...
| --- | --- | --- |
| [`active/PROFESSIONAL_RAW_DEVELOPMENT_PLAN.md`](active/PROFESSIONAL_RAW_DEVELOPMENT_PLAN.md) | Defines the Windows x64 professional camera-RAW architecture: immutable RAW sources, the single unbounded scene-linear working space, owned Rust/WGPU development, colour-managed display/export, dependency evidence gate, and the boundary between calibration data and removed creative LUT features. | Read before adding RAW extensions, decoder dependencies, ICC/DCP parsing, scene-linear surfaces, demosaic/develop controls, monitor transforms, or RAW export. Start with R0 evidence; do not vendor a decoder or expose RAW import before the gate passes. |
...
- Professional RAW development is now an active product direction. Its first
  implementation task is the R0 Windows decoder/metadata evidence gate; no RAW
  dependency or product extension is approved before that comparison record.
