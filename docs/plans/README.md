...
| [`active/CLIP_KEYFRAME_MOTION_PIPELINE_PLAN.md`](active/CLIP_KEYFRAME_MOTION_PIPELINE_PLAN.md) | Defines the performance-first clip keyframe/motion pipeline: Rust-canonical evaluation (landed, #612), then property application in export, GPU preview parity, easing curves, and timeline keyframe lanes — explicitly dropping PR/Resolve legacy baggage. | Use before touching clip property evaluation, `timeline_export` frame passes, or preview compositing of transform/crop/opacity. |
...
| [`active/EDGE_ROUTING_VISUAL_SYSTEM_PLAN.md`](active/EDGE_ROUTING_VISUAL_SYSTEM_PLAN.md) | Defines the single-cut 45° chamfer wire style and edge visual states for the node canvas. Steps 1–6 plus the selected state have landed; hover/running/error states, path caching, and LOD remain. | Use before changing edge rendering, edge states, or connection-drag visuals in `@hgripe/flow`. |
| [`active/LOCAL_REACT_FLOW_FORK_PLAN.md`](active/LOCAL_REACT_FLOW_FORK_PLAN.md) | Defines how to locally own the React Flow / XYFlow graph layer without conflicting with WGPU viewports or GPU/AMD compatibility. | Use before vendoring, wrapping, trimming, or replacing `@xyflow/react`. |
