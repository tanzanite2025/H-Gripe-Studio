...
| [`active/EDGE_ROUTING_VISUAL_SYSTEM_PLAN.md`](active/EDGE_ROUTING_VISUAL_SYSTEM_PLAN.md) | Defines the single-cut 45° chamfer wire style and edge visual states for the node canvas. Steps 1–6, selected state, path caching, and LOD simplification have landed; hover/running/error states plus explicit bend/tidy routing remain. | Use before changing edge rendering, edge states, or connection-drag visuals in `@hgripe/flow`. |
...
- `LOCAL_REACT_FLOW_FORK_PLAN.md` is partially landed and still guides measured
  product trimming and graph-specific optimization.
