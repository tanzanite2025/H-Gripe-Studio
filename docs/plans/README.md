...
| [`active/RUN_SCOPE_AND_EXECUTION_AFFORDANCE_PLAN.md`](active/RUN_SCOPE_AND_EXECUTION_AFFORDANCE_PLAN.md) | Defines row/card/selection/run-to/downstream execution scopes without introducing execution zones or execution nodes too early. | Use before adding row run buttons, card run buttons, selection run, or any execution-zone/trigger-node feature. |
