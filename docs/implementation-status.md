...
| Image Enhance (`studio/image_enhance_cpu.rs` + `image_enhance_onnx.rs`) | Landed (native, optional weight) | Complete CPU Lanczos/denoise/unsharp baseline plus opt-in native Real-ESRGAN x4v3: strict FP32 NCHW contract, padded tiled inference, exact-size resample, managed weight resolution, shared ORT warm session and complete CPU fallback. The third-party ONNX verification artifact is not bundled by default and requires release provenance review. See §2. |
...
Match Light & Color restores `onnx_harmonize` with a native PCT-Net block, and
Image Enhance restores `realesrgan` with a native tiled x4v3 block. All four
reuse the local ORT runtime/session infrastructure and preserve their
native baseline outputs through reported weight, runtime, session, inference,
panic, and provider fallback. Deleted `ccsr` / `supir` workflow values now
degrade to CPU with a migration reason; Detail Repaint still falls back to its
provider path.

| Item | Status | What's missing |
| --- | --- | --- |
| **Super-resolution** backend (Image Enhance) | Landed (native, optional weight) | Native Rust/ORT `realesrgan` uses the FP32 general x4v3 contract, a 128px tile core plus 16px halo, a bounded 4x intermediate and one exact target resample. Alpha/ICC/DPI stay on the shared native path; missing/invalid weight, runtime/session/tensor/inference/panic, non-enlarging targets and provider/precision downgrade are reported without losing the complete CPU result. The pinned third-party ONNX is verification-only pending reproducible lineage review. |
...
| Per-card `engine` seams (matcher) | Partial | Image Enhance, Refine Mask Edge, Detail Watchdog, and Match Light & Color now have native ORT backends behind their existing seams. Detail Repaint still exposes only its provider path after the Python ML backends were deleted. |
...
| **Optional model bundling** (Issue #2) | Planned | BiRefNet lite, SAM 2, ViTMatte, PCT-Net, and Real-ESRGAN are not default installer weights. PCT-Net and the current Real-ESRGAN ONNX verification artifacts additionally require fresh provenance/license review and preferably reproducible exports before any release bundle. |
...
| **Real-ESRGAN super-resolution real inference** | Landed (opt-in CI) | A manual Windows job fetches the pinned x4v3 ONNX re-host, verifies `4871181` bytes and its SHA-256, then runs `realesrgan_inference_when_weight_present` across a real tile boundary. Normal runs cover NCHW packing, tile/halo assembly, strict output validation, limits and CPU fallback while skipping real inference without the optional weight. This gate is integration evidence, not release approval. |
