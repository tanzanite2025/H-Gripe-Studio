...
| **Capability probe / weight cache** | Partial | `probe_engines` reports native `onnx_matting`, `onnx_defect`, and `onnx_harmonize` weight/runtime presence alongside the always-on baselines; sessions validate on first use. The engines honour env overrides, persisted model paths, shared configured caches, and bundled resources. Current ORT is CPU-only; later Windows work must add NVIDIA CUDA and AMD/Intel DirectML. Still open: an Inspector consumer and a replacement for the legacy Dashboard model-management surface. |
