...
- The right rail (`.mask-edit-controls`) holds two tabbed groups (`.mask-panel-group`): top = Tool options / Mask info, bottom (`.grow`) = Layers / History. One panel visible per group; the per-panel `<section class="mask-panel">` headers are CSS-hidden inside groups — a visible duplicate header inside a tab body means the hide rule broke.
